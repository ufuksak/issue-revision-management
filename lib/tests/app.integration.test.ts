...
import app from "./app";
import { sequelize, Issue, Revision } from "../models";
...
describe("Issues IT", () => {
  ...
  test("GET /issues/:id/revisions/diff?revA=1&revB=3 with valid token", async () => {
    ...
    const revision1:any = await Revision.create({
      title: "",
      description: "IT does not generate revision",
      state: "Open",
      issueId: issue.id,
    });
    const revision2:any = await Revision.create({
      title: "",
      description: "IT generated revisions without author",
      state: "In Progress",
      issueId: issue.id,
    });
    const revision3:any = await Revision.create({
      title: "",
      description: "IT generated revisions without author",
      state: "Closed",
      issueId: issue.id,
  ...
  test("GET /issues with valid token verify the issues found", async () => {
    ...
    const issue2:any = await Issue.create({
      title: "Some title2",
